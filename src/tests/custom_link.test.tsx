...
describe('CustomLink', () => {
  ...
  it('fix broken link', async () => {
    ...
    const link = await screen.findByRole('link');
    expect(link).toHaveProperty('href', `https://wikipadia.ru/${encodeURIComponent('имя_(значение)')}`);
  ...
  it('link without host', async () => {
    ...
    const link = await screen.findByRole('link');
    expect(link).toHaveProperty('href', 'https://forum.mista.ru/users.php?id=111');
